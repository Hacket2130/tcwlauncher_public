...
backend="TCWA3-Backend-${version}.zip"

echo "Preparing TCWA3 public deploy ${version}"
mkdir -p "${work}/downloads" "${work}/backend" "${root}/public/downloads" "${root}/backups"
...
curl -fL -o "downloads/${backend}" "${repo_raw}/downloads/${backend}"

grep "${version}" SHA256SUMS.txt > SHA256SUMS-${version}.txt
sha256sum -c SHA256SUMS-${version}.txt
unzip -oq "downloads/${backend}" -d "${work}/backend"
...
tar -czf "${root}/backups/public-release-before-${version}-${stamp}.tar.gz" \
  -C "${root}" release.json server.py test_server.py public/index.html public/profile.html public/servers.html public/community-manager.html public/marketplace.html 2>/dev/null || true

cp "backend/server.py" "${root}/server.py"
cp "backend/test_server.py" "${root}/test_server.py"
